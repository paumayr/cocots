            ...
            (function (coco) {
                ...
                var Action = (function () {
                    ...
                    Action.prototype.AddTargets = function (a) {
                        ...
                        if (a.tc == coco.Node.contextTrans) {
                            this.tc = coco.Node.contextTrans;
                        }
                    };

                    Action.prototype.Symbols = function (tab) {
                        var s;
                        if (this.typ == coco.Node.clas)
                            s = tab.CharClassSet(this.sym).Clone(); else {
                            s = new CharSet();
                            s.Set(this.sym);
                        }
                        return s;
                ...
                var DFA = (function () {
                    ...
                    DFA.prototype.NewTransition = function (from, to, typ, sym, tc) {
                        ...
                        a.target = t;
                        from.AddAction(a);
                        if (typ == coco.Node.clas) {
                            this.curSy.tokenKind = coco.Symbol.classToken;
                        }
                    };
                    ...
                    DFA.prototype.DeleteRedundantStates = function () {
                        ...
                        var used = new coco.BitArray(this.lastStateNr + 1, false);
                    ...
                    DFA.prototype.Step = function (from, p, stepped) {
                        ...
                        switch (p.typ) {
                            ...
                            case coco.Node.iter: {
                                ...
                                this.Step(from, p.sub, stepped);
                                if (p.state != from) {
                                    this.Step(p.state, p, new coco.BitArray(this.tab.nodes.length, false));
                                }
                                break;
                            }
                            case coco.Node.opt: {
                    ...
                    DFA.prototype.NumberNodes = function (p, state, renumIter) {
                        ...
                        if (p.state != null)
                            return;
                        if (state == null || (p.typ == coco.Node.iter && renumIter))
                            state = this.NewState();
                        p.state = state;
                        if (coco.Tab.DelGraph(p)) {
                            state.endOf = this.curSy;
                        }
                    ...
                    DFA.prototype.ConvertToStates = function (p, sym) {
                        ...
                        this.FindTrans(p, true, new coco.BitArray(this.tab.nodes.length, false));
                        if (p.typ == coco.Node.iter) {
                            this.Step(this.firstState, p, new coco.BitArray(this.tab.nodes.length, false));
                        }
                    };
                    ...
                    DFA.prototype.MatchLiteral = function (s, sym) {
                        ...
                        for (; i < len; i++) {
                            ...
                            this.NewTransition(state, to, coco.Node.chr, s[i], coco.Node.normalTrans);
                            state = to;
                        }
                        ...
                        if (state.endOf == null) {
                            state.endOf = sym;
                        } else if (matchedSym.tokenKind == coco.Symbol.fixedToken || (a != null && a.tc == coco.Node.contextTrans)) {
                            // s matched a token with a fixed definition or a token with an appendix that will be cut off
                            this.parser.SemErr("tokens " + sym.name + " and " + matchedSym.name + " cannot be distinguished");
                        } else {
                            matchedSym.tokenKind = coco.Symbol.classLitToken;
                            sym.tokenKind = coco.Symbol.litToken;
                        }
                    };
                    ...
                    DFA.prototype.SplitActions = function (state, a, b) {
                        ...
                        if (seta.Equals(setb)) {
                            ...
                            c = new Action(0, 0, coco.Node.normalTrans);
                            c.AddTargets(a);
                            c.AddTargets(b);
                    ...
                    DFA.prototype.Overlap = function (a, b) {
                        var seta;
                        var setb;
                        if (a.typ == coco.Node.chr) {
                            if (b.typ == coco.Node.chr) {
                                return a.sym == b.sym;
                            } else {
                    ...
                    DFA.prototype.PrintStates = function () {
                        ...
                        for (var state = this.firstState; state != null; state = state.next) {
                            ...
                            for (var action = state.firstAction; action != null; action = action.next) {
                                ...
                                if (action.tc == coco.Node.contextTrans) {
                    ...
                    DFA.prototype.FindAction = function (state, ch) {
                        for (var a = state.firstAction; a != null; a = a.next)
                            if (a.typ == coco.Node.chr && ch.charCodeAt(0) == a.sym)
                                return a; else if (a.typ == coco.Node.clas) {
                    ...
                    DFA.prototype.GetTargetStates = function (a) {
                        ...
                        ret.endOf = null;
                        ret.ctx = false;
                        ret.targets = new coco.BitArray(this.maxStates, false);
                    ...
                    DFA.prototype.CommentStr = function (p) {
                        var s = new coco.StringBuilder("");
                        while (p != null) {
                            if (p.typ == coco.Node.chr) {
                                s.appendNumber(p.val);
                            } else if (p.typ == coco.Node.clas) {
                    ...
                    DFA.prototype.GenLiterals = function () {
                        ...
                        for (var i = 0; i < allSym.length; i++) {
                            var sym = allSym[i];
                            if (sym.tokenKind == coco.Symbol.litToken) {
                    ...
                    DFA.prototype.WriteState = function (state) {
                        ...
                        if (endOf == null) {
                            ...
                            if (endOf.tokenKind == coco.Symbol.classLitToken) {
